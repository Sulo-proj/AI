...
class Preprocessing:
    ...
    def quanQual(data):
        ...
        quan = data.select_dtypes(include=['int64', 'float64']).columns
        qual = data.select_dtypes(include=['object', 'category']).columns

        # quan = [col for col in data.columns if data[col].dtype != 'object']
        return quan, qual    
    
    def quanVariables(data, qual):

        data[qual] = data[qual].apply(lambda x: x.str.strip().str.lower())
        data[qual] = data[qual].replace(['?', 'na', 'n/a', ''], pd.NA)
        data[qual] = data[qual].astype('category')
...
class NormalDistribution:
    ...
    def get_pdf_probability(dataset, startrange, endrange, perc_lookup):
    
        # Plots the histogram and KDE (Kernel Density Estimate) of a dataset using Seaborn.
        # Highlights a given range (startrange to endrange) with vertical lines.
        # Calculates the probability density values of a fitted normal distribution between the given range.
        # Returns the sum of those density values
        
        # ECDF stands for Empirical Cumulative Distribution Function - It is like “percentile lookup”:
        # It’s a function that, given a dataset, tells you the proportion of values less than or equal to a given number.


        ax = sns.histplot(dataset,kde=True,facecolor='green',color='blue')
        ax.axvline(startrange,color='Red')
        ax.axvline(endrange,color='Red')
        ...
        appx_prob=sum(probabilities)
        actual_prob = dist.cdf(endrange) - dist.cdf(startrange)
        
        ecdf = ECDF(dataset)

        return sample_mean, sample_std, appx_prob, actual_prob, ecdf


    def stdNBgraph(dataset, mean, std):
        ...
        sns.histplot(z_score,kde=True,facecolor='green',color='blue')

        avg_z = sum(z_score)/len(z_score)
        std_z = pd.Series(z_score).std()
